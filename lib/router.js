...
Router.route('/showridesto/:to/:who',
       {name:"showRidesTo",
       template:"showRidesTo",
        data: function(){
          const c = Rides.find({to:this.params.to});
          console.dir(c);
          return {rides:c, who:this.params.who};
        }
       }
);
...
Router.map(function () {
  this.route('carpoolFile', {
    path: '/dump/',
    where: 'server',

    action: function () {

      const theRides = Rides.find().fetch();

      this.response.writeHead(200, {'Content-Type':
                                    'application/json; charset=utf-8'});
      this.response.end(JSON.stringify(theRides)+"\n");
...
this.route('carpoolFind', {
  ...
  action: function () {
    ...
    const theRides = Rides.find({to:to}).fetch();
